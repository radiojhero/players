...
function analyseAudio(event?: AudioAnalysis) {
  ...
  visibleCanvases.forEach((canvas) => {
    ...
    for (let index = 0; index < frequenciesNumber; index++) {
      const sliceIndex = Math.floor((index * slicesNumber) / frequenciesNumber);
    ...
    sliceValues.forEach((value, index) => {
      ...
      const y = Math.round((height * value) / (sliceDivisors[index] * 512)) * 2;
      ...
      if (peaks[index]) {
        ...
        canvasContext.lineTo(x + sliceWidth - 1, height - peaks[index] + 0.5);
